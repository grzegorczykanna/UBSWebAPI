...
class APITestCase(unittest.TestCase):
    ...
    def test_get_10_biggest_countries_by_area_for_region_csv(self):
        ...
        response = self.app.get(
            "/api/region/europe/biggest_countries_in_region?format=csv"
        )
        # Check that the status code is 200 (OK)
        self.assertEqual(response.status_code, 200)
    ...
    def test_region_not_found(self):
        ...
        response = self.app.get(
            "/api/region/nonexistentregion/biggest_countries_in_region"
        )
    ...
    def test_subregion_not_found_borders(self):

        response = self.app.get("/api/subregion/nonexistentsubregion/countries_borders")
        self.assertNotEqual(response.status_code, 200)
    ...
    def test_get_the_population_for_subregion_json(self):
        ...
        response = self.app.get("/api/subregion/central%20europe/subregion_population")
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_the_population_for_subregion_csv(self):
        ...
        self.assertIn("Area", country)
        self.assertIn("Borders", country)
        self.assertIn("Total population of Central Europe", country)

    def test_subregion_not_found_total_population(self):

        response = self.app.get(
            "/api/subregion/nonexistentsubregion/subregion_population"
        )
        self.assertNotEqual(response.status_code, 200)
